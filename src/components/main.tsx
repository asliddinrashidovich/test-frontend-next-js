...
function MainPage() {
    ...
    const firstDataClass = (s: number) => {
        return "rounded-[20px] cursor-pointer md:col-span-3 md:col-start-[0] md:row-start-[0] md:rounded-[34px] flex flex-row md:flex-col items-center gap-[30px] min-[375px]:gap-[50px] bg-[#313637] border-[2px]  relative pb-[20px] md:pb-[30px] pl-[20px] min-[375px]:pl-[30px] md:pl-[122px] pr-[20px] min-[375px]:pr-[30px] md:pr-[80px] pt-[20px] md:pt-[34px]   md:mb-[14px]" + (selectedItem === s ? " border-[#FDB056]" : " border-[#484D4E]")
    }
    const otherClass = (s: number) => {
        return "md:row-start-[2] cursor-pointer rounded-[20px] md:rounded-[40px] flex flex-row md:flex-col items-center gap-[30px] min-[375px]:gap-[50px] bg-[#313637] border-[2px] border-[#484D4E] relative pb-[20px] md:pb-[23px] px-[30px] md:px-[18px] pt-[20px] md:pt-[70px] w-full" + (selectedItem === s ? " border-[#FDB056]" : " border-[#484D4E]")
    }
  ...
  return (
    <div className='px-[16px] md:px-[40px] bg-[#232829] pt-[20px] sm:pt-[50px] pb-[30px] md:pb-[150px]'>
        <div className='max-w-[1216px] mx-auto'>
            <h1 className='font-[700] text-[22px] min-[375px]:text-[24px] sm:text-[40px] leading-[110%] text-[#fff] mb-[30px] md:mb-[110px]'>Выбери подходящий для себя <span className='text-[#FDB056]'>тариф</span></h1>
            <div className='flex flex-col min-[1200px]:flex-row items-center gap-[30px] md:gap-[87px] '>
                ...
                <form onSubmit={handleSubmit} className='mb-[66px]'>
                    <div className='flex  flex-col-reverse md:grid   md:grid-cols-3 gap-[6px] min-[375px]:gap-[8px] md:gap-[14px] mb-[10px] min-[375px]:mb-[12px] md:mb-[20px]'>
                        {data.map((item, index) => {
                            return (
                                <div onClick={() => setSelectedItem(index)} className={`${item.is_best  ? firstDataClass(index): otherClass(index)}`} key={item.price}>
                                    ...
                                    <div className={item.is_best ? secondItemClass : firstItemClass}>
                                        <div className=''>
                                            <h3 className='text-[#FFFFFF] font-[500] text-[16px] min-[375px]:text-[18px] md:text-[26px] leading-[120%] text-center mb-[12px] min-[375px]:mb-[16px]'>{item.period}</h3>
                                            <h2 className={`text-[30px] min-[375px]:text-[34px] md:text-[50px] ${index == selectedItem ? "text-[#FDB056] " : "text-[#fff] "} font-[600] leading-[100%] text-nowrap`}>{timeLeft != 0 ? item.price : item.full_price} ₽</h2>
                                        ...
                                        <p className='text-[#fff] font-[400] leading-[130%] text-[14px] min-[375px]:text-[16px]'>{item.text}</p>
                                    </div>
                                </div>)
                        })}
                    </div>
                    ...
                    <button type='submit' className='flex text-[18px] md:text-[20px] font-[700] text-[#191E1F] w-full md:w-auto text-center md:px-[137px] py-[16px] min-[375px]:py-[20px] bg-[#FDB056] rounded-[20px] cursor-pointer mb-[14px] btn' id="hat_btn">
                        <span className='z-[20] text-center w-full' id="hat_text">Купить</span>
                    </button>
            ...
            <div className='border-[1px] border-[#484D4E] p-[12px] md:p-[20px] rounded-[30px] '>
                <div className='border-[1px] border-[#81FE95] bg-[#2D3233] py-[10px] md:py-[16px] px-[18px] md:px-[30px] rounded-[30px] inline-block mb-[30px]'>
                    <p className='text-[#81FE95] text-[16px] min-[375px]:text-[18px] md:text-[28px] font-[500]'>гарантия возврата 30 дней</p>
                </div>
                <p className='text-[#DCDCDC] text-[13px] min-[375px]:text-[14px] md:text-[24px] font-[400]'>Мы уверены, что наш план сработает для тебя и ты увидишь видимые результаты уже через 4 недели! Мы даже готовы полностью вернуть твои деньги в течение 30 дней с момента покупки, если ты не получишь видимых результатов.</p>
            </div>
        </div>
    </div>
  )
